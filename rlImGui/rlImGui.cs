...
namespace rlImGui_cs
{
    public static class rlImGui
    {
        ...
        internal static bool rlImGuiIsControlDown() { return Input.IsKeyDown(KeyboardKey.RightControl) || Input.IsKeyDown(KeyboardKey.LeftControl); }
        internal static bool rlImGuiIsShiftDown() { return Input.IsKeyDown(KeyboardKey.RightShift) || Input.IsKeyDown(KeyboardKey.LeftShift); }
        internal static bool rlImGuiIsAltDown() { return Input.IsKeyDown(KeyboardKey.RightAlt) || Input.IsKeyDown(KeyboardKey.LeftAlt); }
        internal static bool rlImGuiIsSuperDown() { return Input.IsKeyDown(KeyboardKey.RightSuper) || Input.IsKeyDown(KeyboardKey.LeftSuper); }
        ...
        public static unsafe void ReloadFonts()
        {
            ...
            io.Fonts.GetTexDataAsRGBA32(out nint pixels, out width, out height, out bytesPerPixel);
        ...
        unsafe internal static string rImGuiGetClipText(IntPtr userData)
        {
            return Window.GetClipboardText();
        }

        unsafe internal static void rlImGuiSetClipText(IntPtr userData, string text)
        {
            Window.SetClipboardText(text);
        }

        private unsafe delegate string GetClipTextCallback(IntPtr userData);
        private unsafe delegate void SetClipTextCallback(IntPtr userData, string text);
        ...
        private static void NewFrame(float dt = -1)
        {
            ...
            {
                int monitor = Window.GetCurrentMonitor();
                    ...
                    else
                    {
                        ...
                        if ((io.ConfigFlags & ImGuiConfigFlags.NoMouseCursorChange) == 0)
                        {
                            ...
                            if (!MouseCursorMap.ContainsKey(imgui_cursor))
                                Input.SetMouseCursor(MouseCursor.Default);
                            else
                                Input.SetMouseCursor(MouseCursorMap[imgui_cursor]);
                        }
                    }
                }
            }
        ...
        private static void HandleGamepadStickEvent(ImGuiIOPtr io, GamepadAxis axis, ImGuiKey negKey, ImGuiKey posKey)
        {
            ...
            float axisValue = Input.GetGamepadAxisMovement(0, axis);
